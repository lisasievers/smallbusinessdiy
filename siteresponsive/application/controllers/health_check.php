...
class health_check extends Home
{
    ...
    public function report($id=0,$domain="")
    {
       ...
       $data['topmenu']=array('Home' => 'http://localhost/sitehome','About Us' => 'http://localhost/sitehome/#aboutus','Products' => 'http://localhost/sitehome/#product','Resources' => 'http://localhost/sitehome/#resource','Blog' => 'http://localhost/sitehome/blog');
       $data['bmenu1']=array('Account' => 'http://localhost/sitehome/#','Support' => 'http://localhost/sitehome/#','Product Catalog' => 'http://localhost/sitehome/#');
       $data['bmenu2']= array('Sitemap' => 'http://localhost/sitehome/#','Find Domain' => 'http://localhost/sitehome/#','Whois Search' => 'http://localhost/sitehome/#');
    ...
    public function have_site_users()
    {
        ...
        $data = array(
            ...
            //$file_name=$this->report_responsive($id);
          //  $details_url=site_url('health_check/report_responsive'."/".$last_id.'/'.$this->site_check->clean_domain_name($domain));
            //$download_link=base_url().$file_name;
            //echo '<div class="box-body chart-responsive minus"><div class="col-xs-12"><div class="alert text-center" style="font-size:18px">'.$this->lang->line("pdf report has been generated"). '<br/> <br/><a href="'.$download_link.'" target="_BLANK" style="font-size:20px"> <i class="fa fa-cloud-download"></i> '.$this->lang->line("click here to download").'</a></div></div></div>';
           ...
           // $mesg = $this->load->view('template/email','',true);
                ...
                $from = $this->config->item('institute_email');
                $subject = 'Have site user registration';
                //$message=$file_name;
                $message = 'Hi, '.$resname.'<br/>Your Registered Email: ' .$resemail. '<br/>Your Temporary password: password<br/>Please signin and get more reports <a href="http://localhost/sitehome/">Sign In</a>';
                $this->_mail_sender($from, $to = $resemail, $subject, $message, $mask = $from,$html=1);
                $this->session->set_userdata('mail_sent', 1);
