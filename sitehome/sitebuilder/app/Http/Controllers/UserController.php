...
/* cooke storage */
 ...
 //Cookie::make('rts', $email, 360,$path = null, $domain = 'http://glimpsesvr.com');
  			...
  			$mdata = array('from'=>'acethamarai@gmail.com','aname'=>'SMBDIY Team','name'=>$first_name.' '.$last_name, 'email'=>$email);
            ...
            {
                $message->to( 'thamaraiselvan@wemagination.net')->from( $mdata['from'], $mdata['aname'] )->subject( 'SMBDIY Signup' );
            }); 
	...
	{
		 //$cookieJar->forget(cookie(string 'emailid',string $path='/',string $domain=localhost));
		$request->session()->flush();
		Auth::logout();
		//Cookie::queue(Cookie::forget('emailid'));
		
		return redirect()->route('home')->withCookie(Cookie::forget('emailid'));
	}
